...
/**
 * Original Author: Hassy Veldstra (h@artillery.io)
 * 
 * Modified By: Sasa Savic (savic.s82@gmail.com), marked MODIFIED_SS
 */
...
function ExpectationsPlugin(script, events) {
  ...
  script.scenarios.forEach(function (scenario, idx) {
    ...
    /**
     * MODIFIED_SS
     * In order to inject scenario such that we know which scenario the errors are coming from
     * we need to create a function per scenario
     */
    scenario.afterResponse.push(`expectationsPluginCheckExpectations${idx}`);
    script.config.processor[`expectationsPluginCheckExpectations${idx}`] = expectationsPluginCheckExpectationsWithScenario(scenario);
 ...
 * @param {*} scenario 
 */
function expectationsPluginCheckExpectationsWithScenario(scenario) {

  return function expectationsPluginCheckExpectations(
    req,
    res,
    userContext,
    events,
    ...
    if (failedExpectations) {
      return done(new Error(`Failed ${scenario.name}`));
    } else {
      return done();
    }
  }
